...
onSnapshot(salesQuery, (snapshot) => {
    ...
    // KPI: Avg Monitoring should be the average monitoring fee of ALL sales in the current calendar month
    let monthMonitoringSum = 0;
    ...
    snapshot.forEach(doc => {
        ...
        if (isThisMonth(ts)) {
            ...
            monthSaleCount++;
            monthMonitoringSum += Number(data.monitoring || 0);
        }
        if (isThisYear(ts)) {
    ...
    let avgMonitoring = monthSaleCount ? (monthMonitoringSum / monthSaleCount) : 0;
